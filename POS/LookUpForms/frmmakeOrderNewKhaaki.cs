    ...
    new { Text = "Female", Value = "2" }
        ...
        public void SetDetailBeforeSave()
        {
            ...
            if (this.Saleview.Rows.Count > 0)
            {
                ...
                {
                    var ItemId = Convert.ToString(Saleview.Rows[i]["ItemId"]);
                    var ItemNumber = Convert.ToString(Saleview.Rows[i]["ItemNumber"]);
                    var ItenName = Convert.ToString(Saleview.Rows[i]["ItenName"]);
                    var ItemRate = Convert.ToString(Saleview.Rows[i]["ItemRate"]);
                    var TotalQuantity = Convert.ToString(Saleview.Rows[i]["TotalQuantity"]);
                    var Test = Convert.ToString(Convert.ToDecimal(Saleview.Rows[i]["ItemRate"]) * Convert.ToDecimal(Saleview.Rows[i]["TotalQuantity"]));
                    var TaxPercentage = Convert.ToString(Saleview.Rows[i]["TaxPercentage"]);
                    var TaxAmount = Convert.ToString(Saleview.Rows[i]["TaxAmount"]);
                    var DiscountPercentage = Convert.ToString(Saleview.Rows[i]["DiscountPercentage"]);
                    var DiscountAmount = Convert.ToString(Saleview.Rows[i]["DiscountAmount"]);
                    var TotalAmount = Convert.ToString(Saleview.Rows[i]["TotalAmount"]);
                    var ABC = "0";
                    var MinQUantity = Convert.ToString(Convert.ToDecimal(Saleview.Rows[i]["MinQuantity"]));
                    var SchemeID = Convert.ToString(Saleview.Rows[i]["SchemeID"]);
                    var ISExchange = Convert.ToString(Saleview.Rows[i]["isExchange"]);
                    string[] row = {
                            Convert.ToString(Saleview.Rows[i]["ItemId"]),
                            Convert.ToString(Saleview.Rows[i]["ItemNumber"]),
                            Convert.ToString(Saleview.Rows[i]["ItenName"]),
                            Convert.ToString(Saleview.Rows[i]["ItemRate"]),
                            Convert.ToString(Saleview.Rows[i]["TotalQuantity"]),
                            Convert.ToString(Convert.ToDecimal(Saleview.Rows[i]["ItemRate"])*Convert.ToDecimal(Saleview.Rows[i]["TotalQuantity"])),
                            Convert.ToString(Saleview.Rows[i]["TaxPercentage"]),
                            Convert.ToString(Saleview.Rows[i]["TaxAmount"]),
                            Convert.ToString(Saleview.Rows[i]["DiscountPercentage"]),
                            Convert.ToString(Saleview.Rows[i]["DiscountAmount"]),
                            Convert.ToString(Saleview.Rows[i]["TotalAmount"]),
                            "0",
                            Convert.ToString(Convert.ToDecimal(Saleview.Rows[i]["MinQuantity"])),
                            Convert.ToString(Saleview.Rows[i]["SchemeID"]),
                            Convert.ToString(Saleview.Rows[i]["isExchange"]),
                    };
                    ItemSaleGrid.Rows.Add(row);
                    CalculateDetail();
                    ItemSaleGrid.ReadOnly = true;
                    panel1.Enabled = true;
